...
class UKAN(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3, embed_dims=[256, 320, 512], no_kan=False, attention_mode='none',
    drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1], **kwargs):
        super().__init__()

        valid_attention_modes = {'none', 'se_only', 'cbam_only', 'cbam_se'}
        if attention_mode not in valid_attention_modes:
            raise ValueError(f'Unsupported attention_mode: {attention_mode}. Supported: {sorted(valid_attention_modes)}')
        self.attention_mode = attention_mode
        use_cbam = attention_mode in {'cbam_only', 'cbam_se'}
        use_se = attention_mode in {'se_only', 'cbam_se'}
        ...
        self.enc_cbam1 = CBAM(kan_input_dim//8) if use_cbam else nn.Identity()
        self.enc_cbam2 = CBAM(kan_input_dim//4) if use_cbam else nn.Identity()
        self.enc_cbam3 = CBAM(kan_input_dim) if use_cbam else nn.Identity()
        ...
        self.skip_cbam4 = CBAM(embed_dims[1]) if use_cbam else nn.Identity()
        self.bottleneck_se = SEBlock(embed_dims[2]) if use_se else nn.Identity()
        self.dec_se3 = SEBlock(embed_dims[0]//4) if use_se else nn.Identity()
        self.dec_se4 = SEBlock(embed_dims[0]//8) if use_se else nn.Identity()
    ...
    def forward(self, x):
        ...
        out = self.enc_cbam1(out)
        t1 = out
        ### Stage 2
        out = F.relu(F.max_pool2d(self.encoder2(out), 2, 2))
        out = self.enc_cbam2(out)
        t2 = out
        ### Stage 3
        out = F.relu(F.max_pool2d(self.encoder3(out), 2, 2))
        out = self.enc_cbam3(out)
        t3 = out
        ...
        out = self.dec_se3(out)
        out = F.relu(F.interpolate(self.decoder4(out),scale_factor=(2,2),mode ='bilinear'))
        out = torch.add(out,t1)
        out = self.dec_se4(out)
